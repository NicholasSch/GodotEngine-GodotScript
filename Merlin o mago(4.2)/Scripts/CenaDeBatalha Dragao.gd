...
var num1 = rng.randi_range(1,500)
var num2 = rng.randi_range(1,20)
...
var Pergunta = String(num1)+" + "+String(num2)+ "÷"+String(num3)
...
func aleatorizarpergunta():
	while resposta<0:
		arr = rng.randomize()
		num1 = rng.randi_range(1,500)
		num2 = rng.randi_range(1,20)
	...
	while fire<0 or fu<0 or temp<0 or trov<0 or fire==fu or fire == temp or fire == trov or fu == temp or fu == trov or temp == trov:
		...
		temp = resposta + rng.randi_range(-20,20)
		trov = resposta + rng.randi_range(-20,20)
	Pergunta = String(num1)+" + "+String(num2)+ "÷"+String(num3)

func _on_Timer5_timeout():
	if$Merlim.lives>0 and $Dragao.hearts>0:
		$Merlim.powers = true
	elif $Dragao.hearts>0:
