...
var num3 = rng.randi_range(0,100)
var numr = rng.randi_range(0,3)
var resposta =-1
var fire = -1
var fu = resposta + rng.randi_range(-20,20)
var temp = resposta + rng.randi_range(-20,20)
var trov = resposta + rng.randi_range(-20,20)
var Pergunta = String(num1)+" X "+String(num2)+" + "+String(num3)
...
func atualizavalor():
	if numr == 0 and fu != resposta and temp != resposta and trov != resposta:
		...
		fire = resposta + rng.randi_range(-20,20)
		fu = resposta + rng.randi_range(-20,20)
		temp = resposta + rng.randi_range(-20,20)
		trov = resposta + rng.randi_range(-20,20)
		_ready()
...
func aleatorizarpergunta():
	while resposta<0:
		arr = rng.randomize()
		num1 = rng.randi_range(0,10)
		num2 = rng.randi_range(0,10)
		num3 = rng.randi_range(0,100)
	...
	while fire<0 or fu<0 or temp<0 or trov<0 or fire==fu or fire == temp or fire == trov or fu == temp or fu == trov or temp == trov:
		...
		temp = resposta + rng.randi_range(-20,20)
		trov = resposta + rng.randi_range(-20,20)
	Pergunta = String(num1)+" X "+String(num2)+" + "+String(num3)
func _on_Timer5_timeout():
	if$Merlim.lives>0 and $Morcegosa.hearts>0:
		$Merlim.powers = true
	elif $Morcegosa.hearts>0:
		await get_tree().create_timer(0.2).timeout
		Transicao.fade_in("res://Cenes/level.tscn")
	elif$Merlim.lives>0:
		Global.MorcegosaM=true
		await get_tree().create_timer(0.2).timeout
		Transicao.fade_in("res://Cenes/level.tscn")
