...
func _ready():
	$EsqueletoAnim.play("Esqueleto idle")
	if Global.EsqueletoM==true:
		queue_free()
...
func fire():
	...
	$EsqueletoAnim.play("Ataque")
	$Timer3.start()

func Furacao():
	var fu = furacao.instantiate()
	if $EsqueletoAnim.flip_h==true:
		fu.direction = -1
	else:
		fu.direction=1
	get_parent().add_child(fu)
	fu.position.y=position.y-22
	fu.position.x=position.x-100
	$EsqueletoAnim.play("Ataque")
	$Timer3.start()
